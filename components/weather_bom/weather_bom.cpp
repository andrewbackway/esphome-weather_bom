...
void WeatherBOM::setup() {
  ...
  if (wifi::global_wifi_component != nullptr) {
    wifi::global_wifi_component->add_connect_callback([this]() {
...
void WeatherBOM::update() {
...
#else
  ESP_LOGD(TAG,
           "Starting update. Current geohash: '%s', have_static_lat: %d, "
           "have_static_lon: %d, "
           "have_dynamic: %d, dynamic_lat: %f, dynamic_lon: %f",
           this->geohash_.c_str(), this->have_static_lat_,
           this->have_static_lon_, this->have_dynamic_, this->dynamic_lat_,
           this->dynamic_lon_);
  ...
  if (this->lat_sensor_ != nullptr && this->lon_sensor_ != nullptr &&
      this->have_dynamic_) {
    if (this->dynamic_lat_ != this->last_lat_ ||
        this->dynamic_lon_ != this->last_lon_) {
      ESP_LOGD(TAG,
               "GPS coordinates changed, resetting geohash for re-resolution");
      this->geohash_.clear();
    }
  }
...
void WeatherBOM::do_fetch() {
  ...
  if (this->geohash_.empty()) {
    if (!this->resolve_geohash_if_needed_()) {
      ...
      App.scheduler.set_timeout(0, [this]() { this->running_ = false; });
      return;
    }
  ...
  // Observations
  {
    ...
    if (this->fetch_url_(url, this->obs_body_)) {
      ESP_LOGD(TAG, "Fetched %d bytes for observations",
               (int)this->obs_body_.size());
      success_obs = true;
    } else {
  ...
  // Forecast
  {
    ...
    if (this->fetch_url_(url, this->fc_body_)) {
      ESP_LOGD(TAG, "Fetched %d bytes for forecast",
               (int)this->fc_body_.size());
      success_fc = true;
    } else {
  ...
  // Warnings
  {
    ...
    if (this->fetch_url_(url, this->warn_body_)) {
      ESP_LOGD(TAG, "Fetched %d bytes for warnings",
               (int)this->warn_body_.size());
      success_warn = true;
    } else {
...
static float _coalesce_number(cJSON* obj, const char* k1,
...
static std::string _coalesce_string(cJSON* obj, const char* k1,
                                    const char* k2 = nullptr) {
  if (!obj) return {};
  cJSON* v = cJSON_GetObjectItemCaseSensitive(obj, k1);
...
void WeatherBOM::parse_and_publish_forecast_(const std::string& json) {
  ...
  if (cJSON_IsArray(arr)) {
    ...
    auto handle_day = [&](cJSON* day, bool is_today) {
      ...
      ESP_LOGD(TAG, "%s rain chance: %f, amount: '%s'",
               is_today ? "Today" : "Tomorrow", chance, amount.c_str());

      std::string summary = _coalesce_string(day, "short_text", "summary");
      ESP_LOGD(TAG, "%s summary: '%s'", is_today ? "Today" : "Tomorrow",
               summary.c_str());
      std::string icon = _coalesce_string(day, "icon_descriptor", "icon");
      ESP_LOGD(TAG, "%s icon: '%s'", is_today ? "Today" : "Tomorrow",
               icon.c_str());
